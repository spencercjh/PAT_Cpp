...
double compareAnswer(vector<char> &as,a nswer &rs,int k)//k题号
{
	if(as==rs.ans)
	...
	while(i<as.size()&&j<rs.ans.size())
	{
		...
		else if(as[i]<rs.ans[j])//选错了
		{
			...
			mis[to_string(k+1)+"-"+rs.ans[j++]]++;//少选了
	}
	...
	while(j<rs.ans.size())
		mis[to_string(k+1)+"-"+rs.ans[j++]]++;//其余都少选了
	if(issemiright)
		return 0.5*rs.fullscore;//半对只能拿一半分
	return 0.0;//选错0分
}
int main()
{
